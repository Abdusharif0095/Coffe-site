 let selected_igredients = new Set();
...
let unique_ingredients = new Set();
jsonfile.forEach(
    element=>{
...
function modifySelectedIngredients() {
    unique_ingredients.forEach(ingredient=>{

        let nextIngredient = document.createElement('div');
        nextIngredient.className = "ingredientItem";
        ...
        ingredientIcon.className = "ingredientsIcon";
        ingredientIcon.textContent = ingredient.slice(0, 2);

        ingredientName.appendChild(ingredientIcon);

        let ingredientsLabel = document.createElement('label');
        ingredientsLabel.className = "ingredientLabel";
        ...
        checkbox.onclick = function(){
            if(checkbox.checked) { addSomeIngredient(ingredient); }
            else { removeSomeIngredient(ingredient); }
        }

        nextIngredient.appendChild(ingredientName);
        nextIngredient.appendChild(checkbox);

        let ingredients = document.getElementById("ingredients");
        ingredients.appendChild(nextIngredient);

    });
}

document.addEventListener('DOMContentLoaded', modifySelectedIngredients, false);

function addSomeIngredient(text) {
    selected_igredients.add(text);
    prepareCoffee();
}
function removeSomeIngredient(text)
{
    selected_igredients.delete(text);
    prepareCoffee();
}

function prepareCoffee() {
    let count = selected_igredients.size;
    let сoffeeItem = null;
    let result = true;
    let array = Array.from(selected_igredients);
    jsonfile.forEach(element => {
        if(element.ingredients.length === count) {
